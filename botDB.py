...
async def connect_to_db():
    ...
    async def newUser(self, username: str, user_id: int) -> bool:
        try:
            # with await self.pool_conn() as conn:
    ...
    async def updateMessages(self, username: str, user_id: int) -> bool:
        if not await self.newUser(username, user_id):
            return False
        try:
            conn = await self.connect_to_db()
            await conn.fetch(
    ...
    async def is_location_set(self, user_id: int) -> bool:
        try:
            conn = await self.connect_to_db()
    ...
    async def set_location(self, user_id: int, city_name: str, lat: int, lon: int, hidden: bool) -> str:
        try:
            conn = await self.connect_to_db()
            await conn.execute(
    ...
    async def get_location(self, user_id: int) -> str:
        try:
            with await self.pool_conn() as conn:
    ...
    async def add_trusted_user(self, username: str) -> bool:
        try:
            conn = await self.connect_to_db()
            await conn.fetch(
    ...
    async def get_trusted_users(self) -> [str]:
        try:
            conn = await self.connect_to_db()
    ...
    async def delete_trusted_user(self, username: str) -> bool:
        try:
            conn = await self.connect_to_db()
    ...
    async def insertCaughtPokemon(self, pokemon_id: int, pokemon_name: str, user_id: int, username: str) -> bool:
        try:
            conn = await self.connect_to_db()
            await conn.fetch(
    ...
    async def getEscapePhrase(self, ) -> str:
        try:
            conn = await self.connect_to_db()
    ...
    async def getPokedex(self, username: str) -> str:
        try:
            conn = await self.connect_to_db()
    ...
    async def has_caught(self, user_id: int, mon: str) -> str:
        try:
            conn = await self.connect_to_db()
    ...
    async def remove_all_pokemon(self, user_id: int) -> bool:
        try:
            conn = await self.connect_to_db()
            await conn.fetch(
